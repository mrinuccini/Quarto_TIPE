...
def generate_minmax_tree(root: Node, plateau: Plateau, pioche: list, piece_a_placer: Piece, max_depth: int) -> None:
    ...
    for case in plateau.recuperer_cases_vides():
        ...
        for piece_a_donner in pioche:
            child = Node((clone, piece_a_donner))
            root.insert(child)
...
def minimax(node: Node, maximise: bool) -> None:
    successors = node.get_enfants_directs()
    
    if len(successors) == 0: 
        return evaluate(node.val[0], node.val[1])
    else:
        if maximise:
...
def evaluate(plateau: Plateau, piece_a_donner: Piece):
    ...
    for lcd in lcds:
        if len(lcd) <= 1: # Si la ligne est vide ou n'a qu'une seule pièce, elle ne rapporte rien
            continue
